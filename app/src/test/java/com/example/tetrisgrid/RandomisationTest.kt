...
class RandomisationTest {
    ...
    @Test
    fun `getSelectionProbabilities, it correctly returns the probability a piece will be selected`() {
        val weightedCells = listOf(
            1.0 to listOf(0 to 0),
        ...
        val probabilities = Randomisation.getSelectionProbabilities(weightedCells)
    ...
    @Test
    fun `getSelectionProbabilities, if collection is empty it bails out with an error`() {
        val weightedCells = listOf<Pair<Double, Piece>>()

        val exception = assertThrows(IllegalArgumentException::class.java) {
            Randomisation.getSelectionProbabilities(weightedCells)
        }
    ...
    @Test
    fun `getSelectionProbabilities, if weighedSum is effectively zero it bails out with an error`() {
    ...
    @Test
    fun `getSelectionProbabilities, if weighedSum is negative it bails out with an error`() {
        // Note, the sum of weights here isn't actually 0.0
        val weightedCells = listOf(
